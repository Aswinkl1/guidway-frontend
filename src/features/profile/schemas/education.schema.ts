...
export const CreateEducationSchema = BaseEducationSchema.refine(
  (data) => {
    if (!data.isCurrent) {
...
export const EditEducationSchema = BaseEducationSchema.extend({
  id: z.uuid(),
});

export type EditEducationType = z.infer<typeof EditEducationSchema>;

export const DeleteEducationSchema = EditEducationSchema.pick({ id: true });
